...
namespace Sticky\Controller;

use Zend\Mvc\Controller\AbstractActionController;
use Zend\View\Model\ViewModel;

class StickyController extends AbstractActionController {
    protected $_stickyNotesTable;
    public function getStickyNotesTable() {
        if (!$this->_stickyNotesTable) {
            $sm = $this->getServiceLocator();
            $this->_stickyNotesTable = $sm->get('Sticky\Model\StickyTable');
        }
...
public function addAction() {
        ...
        if ($request->isPost()) {
            $new_note = new \Sticky\Model\Entity\Sticky();
